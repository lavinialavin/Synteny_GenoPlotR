...
##llamar paquetes en libreria
library(ade4)
library(grid)
library(genoPlotR)
##Llamar archivo -directorio
##Llamar archivo -directorio
bone_file <- "/Users/lavinialavin/Desktop/Ejemplos R/Genomica/alin_wc2.backbone"
##Determinar genoma de referencia (genoma mas largo) 
bbone <- read_mauve_backbone(bone_file, ref=10, gene_type = "side_blocks", header = TRUE, filter_low = 0, common_blocks_only = FALSE)#Cambiar common blocks only a FALSE 
#Etiquetar DNA_SEGS obtenidos de mauve, deben corresponder con las etiquetas de guide.tree
...
#calcular longitudes de los genomas 
...
#analisis con guide.tree de Mauve##Llamar arbol/plot con arbol
tree<- "/Users/lavinialavin/Desktop/Ejemplos R/Genomica/alin_wc2.guidetree"
##distancias de WC1 alin_correc
...
#Plot de genes comparados y genoma completo
...
#Plot direcciones flechas para diferenciar bloques
...
#Comparasiones generadas por mauve, analisis entre bloques (dna_segs) ( los analisis son uno menos que el numero de dna_segs)
...
#Plot genomas, sin nombres, ni etiquetas
plot_gene_map(dna_segs=dna_segs, comparisons=comparisons)
#comparaciones con nombres y arbol,names=guide_tree
...
plot_gene_map(bbone$dna_segs=dna_seg, comparisons=comparisons,
              annotations=annot, annotation_height=1.3, tree=tree_demo, tree_width=2, main="Comparison of Homologous Segments in 7 sub-genomic regions")




##B_bass 18kbp, delimitar al tamaño del contig que contiene el gen de interes 
...
#plot sin arbol, regiones y comparaciones; solo se proyectan las flechas que delimitan los bloques calculados 
 plot_gene_map(dna_segs=dna_segs, comparisons=comparisons,
                  annotations=annot, annotation_height=1.3,
                tree=tree_demo, tree_width=2,
                main="Comparison of subgenomic regions centered on wc2")
 
#Plot arbol, lcbs y comparaciones  
plot_gene_map(bbone$dna_segs, bbone$comparisons, tree=tree_demo,
          xlims=xlims,
     limit_to_longest_dna_seg=FALSE,
         dna_seg_scale=TRUE, scale=TRUE,
...
#Plot genoplotr manual 1, determinar region, acotar a menos de 50kbp
##Xlims para 11 genomas 50kbp
...
plot_gene_map(dna_segs=dna_segs, bbone$comparisons, tree=tree_demo, tree_width=2, xlims=xlims,limit_to_longest_dna_seg=FALSE,dna_seg_scale=TRUE, scale = TRUE, main = "Subgenomic comparisons & block identification (frq)")


## Se calculan las anotaciones de bloques, incluir en plot de ser necesario
